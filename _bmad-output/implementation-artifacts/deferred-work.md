...
## CONFIRMED (2026-09-24, điều tra thực nghiệm) — root cause thật của mọi số liệu test backend mâu thuẫn ở spec-3-3, thay thế 2 finding suy đoán phía trên (dòng 383-387, giữ nguyên không xoá theo quy tắc append-only)

- source_spec: `_bmad-output/implementation-artifacts/spec-3-3-xác-nhận-tiếp-nhận-cảnh-báo-ack.md`
  summary: "`installService.test.js` bị FLAKY THẬT (không phải 1 fail ổn định/pre-existing như mọi entry Change Log mô tả) — dựng git worktree riêng, chạy `node --test` nhiều lần LIÊN TIẾP CÙNG 1 COMMIT (`b652faf`, `f64dfeb`): 8 file test khác (178 test) luôn 178/178 tuyệt đối qua mọi lần chạy; riêng `installService.test.js` (9 test) dao động 1/2/2/2/9 rồi 9/9/9/1/2 test được BÁO CÁO (không chỉ pass/fail lật, cả TỔNG SỐ test cũng đổi). Số thật ổn định của toàn bộ backend tại commit cuối Story 3.3 (`f64dfeb`) là **188 test tổng** (187 luôn pass + 9 của installService.test.js), KHÔNG PHẢI 181 như mọi bản spec từng ghi (kể cả '180/181' patch trước đó của epic-3-retro-item-17, đã tự sửa lại). Root cause cơ chế PASS/FAIL: `installService.test.js:83-104`'s test đầu tiên đua giữa 2 `setImmediate` lồng nhau với `startTimeoutMs=50ms` (`installService.test.js:15`, dùng trong `installService.ts:174-182`'s `setTimeout`) — dưới tải CPU thật khi chạy cùng 187 test khác, đôi khi timeout thắng trước khi `setImmediate` kịp chạy. Cơ chế khiến TỔNG SỐ test cũng đổi (không chỉ 1 test pass/fail) chưa xác định chính xác — nghi vấn liên quan cách `node:test` xử lý khi 1 test trong file bị timeout/reject giữa chừng."
  evidence: Thực nghiệm trực tiếp (10 lần chạy, 2 commit khác nhau, worktree cô lập không ảnh hưởng working tree chính) — không phải suy luận qua đọc code. Toàn bộ chuỗi "X/Y test pass" trong Change Log của spec-3-3 (172/173, 186/186, 187/187, [round 3 thiếu], 180/181) đều CHỈ LÀ 1 SNAPSHOT NGẪU NHIÊN của cùng 1 file flaky này — không phải lỗi số học của người/agent ghi lại, và không đáng tin cậy làm cơ sở đối chiếu. Đề xuất fix thật: tăng `SHORT_TIMEOUT_MS` (hiện 50ms, có thể lên 500ms-1s) hoặc bỏ phụ thuộc wall-clock timing thật ở test #1 (dùng fake timer/`node:test`'s mock timer thay vì `setImmediate` đua với `setTimeout` thật). Xem action_item tương ứng trong `sprint-status.yaml`. [`installService.test.js:15,83-104`, `installService.ts:174-182`]

## Deferred from: code review of spec-epic2-item-10-12-don-map-orphan-gop-hang-so-luoi (/bmad-build step-04, 2026-09-24)

- source_spec: `_bmad-output/implementation-artifacts/spec-epic2-item-10-12-don-map-orphan-gop-hang-so-luoi.md`
  summary: `pruneChannel()` (backend, 3 service) và `applyRegistrySnapshot()` (frontend) chỉ có tác dụng cho client MỚI connect/reconnect — 1 tab dashboard đang mở sẵn tiếp tục hiện "ghost channel" đã bị gỡ khỏi registry cho tới khi nó tự reconnect.
  evidence: Blind-hunter phát hiện, đúng như Boundaries của spec này đã chủ đích chọn ("không broadcast, client tự đồng bộ qua registry-snapshot lần connect kế tiếp") để ưu tiên mục tiêu chính (chặn rò rỉ bộ nhớ) — không phải oversight. Cùng gốc với gap đã defer từ Story 2.3 review (`wsUiAdapter.ts` không rebroadcast `registry-snapshot` cho client đang mở khi hot-reload) — chưa giải quyết trải nghiệm UI real-time, chỉ giải quyết rò rỉ bộ nhớ backend.
- source_spec: `_bmad-output/implementation-artifacts/spec-epic2-item-10-12-don-map-orphan-gop-hang-so-luoi.md`
  summary: `EmailAlertAdapter`/`TelegramAlertAdapter` mỗi adapter tự giữ 1 Map `lastSentAt` (cooldown dedup) theo channelId/`channelId:displayState` — cùng nhóm orphan-Map mà spec này fix ở nơi khác, nhưng KHÔNG được wiring vào `registryPort.onEntriesRemoved()` mới.
  evidence: Verification-gap layer phát hiện qua grep toàn bộ Map keyed-by-channelId trong `dashboard-backend/src`. Pre-existing từ Story 4.2/4.3, ngoài Code Map/Intent đã khai báo của spec này (chỉ định rõ 3 service + wsUiAdapter, không nhắc 2 alert adapter) — không phải regression, nhưng đáng gộp vào lần dọn dẹp Map orphan kế tiếp nếu có.
- source_spec: `_bmad-output/implementation-artifacts/spec-epic2-item-10-12-don-map-orphan-gop-hang-so-luoi.md`
  summary: `channelStore.ts`'s `applyRegistrySnapshot()` reset `channelAck: new Map()` vô điều kiện mỗi lần nhận `registry-snapshot` (hành vi có từ Story 3.3, spec này không đổi) — nay registry hot-reload để gỡ kênh là thao tác được kỳ vọng xảy ra thường xuyên hơn, nên mỗi lần đó cũng xoá ack-label của MỌI kênh khác không liên quan, không chỉ kênh bị gỡ.
  evidence: Blind-hunter phát hiện. Dòng code không đổi (pre-existing), nhưng tần suất kích hoạt tăng lên do use-case mới của spec này (gỡ kênh qua hot-reload) — chưa có test nào xác nhận đây vẫn là hành vi mong muốn trong bối cảnh mới.
- source_spec: `_bmad-output/implementation-artifacts/spec-epic2-item-10-12-don-map-orphan-gop-hang-so-luoi.md`
  summary: Không có test cho nhánh watcher phát hiện file `channel-registry.json` bị XOÁ HẲN (event unlink/ENOENT) — `loadAndValidate()` throw, `reload()` rơi vào catch giữ nguyên registry cũ, `onEntriesRemoved` không bao giờ kích hoạt dù về mặt vận hành mọi kênh coi như "biến mất".
  evidence: Blind-hunter phát hiện. Hành vi catch-giữ-nguyên khi lỗi đọc file là pre-existing từ Story 2.2, spec này không đổi nhánh đó — chỉ là gap test-coverage lộ ra rõ hơn khi có thêm ý nghĩa "channel bị gỡ" gắn vào nhánh reload thành công.
- source_spec: `_bmad-output/implementation-artifacts/spec-epic2-item-10-12-don-map-orphan-gop-hang-so-luoi.md`
  summary: 2 test tích hợp mới trong `main.test.ts` (hot-reload gỡ kênh / chỉ đổi metadata) phụ thuộc watcher + debounce THẬT (300ms mặc định, không có seam cấu hình riêng cho registry watcher trong `startApp()`, khác `debounceMs` đã có của `ChannelStateService`) kèm nhiều `setTimeout` cố định (50/100/300ms) để né race — rủi ro flaky tiềm ẩn trên CI/Windows.
  evidence: Blind-hunter phát hiện. Mirror đúng pattern đã có sẵn trong cùng file (`main.test.ts`'s test wiring ack-command/composite alertPort khác) nên không phải sai lệch mới, nhưng thiếu seam cấu hình debounce cho registry watcher là gap pre-existing từ Story 2.2, chưa từng cần tới cho tới khi có test tích hợp thật đầu tiên chạm watcher này.
